...
こうしたレイヤーを作っていくとき、普通は問題領域のそばと、一番下の実装のそばのレイヤーはなんとなくわかることが多い（往々にして最終的にはこの最初の分かってるつもりの物とは似ても似つかない物になりがちだが）。
...
だから最初にただ動くだけのコードは結構抽象化に気を配ってなくても書ける。
...
この時にここまでの抽象化の話がクリティカルに重要になってくる。
...
そしてその言葉で解法を記述する。
この時は「ちょうど良い抽象度の言葉」は使う側の面から意識的に見る。
...
また、現在の最新バージョンじゃなくて、なるべく初期の、バージョン1とかも小さくて良い。

コードを読む時に、ただ読むのでは無くて、この作られている言葉に意識して読んでいくと、
...
なお、この話のもとになっている「暇つぶしCプログラミング教室」も、この中規模の入り口くらいになるように問題を選んでいる。
こちらの誘導がある程度この言葉を作るという事を想定して書いているので、課題をやっていく過程で作るための試行錯誤が出来るんじゃないかと思っている。

### ボトムアップに考える場合でも違いはある

中規模の問題が初めて解けるようになる時というのは、だいたいトップダウンに考える技術を身に着けた時だろう。
...
例えばJava, C, C++, C#, JS, go, MLあたりはトップダウンに書きやすい気がする。
PowerShell, R, ruby, LispやSchemeあたりはボトムアップに書きやすい。
...
理由は良くわからないが、結局完全には問題は分離出来ていないので、全体の理解がある程度は必要なんだと思う。
...
それは書いている人が素人なのじゃなくて、一段難しい問題に取り組んだ結果の場合がある（単にへぼいプログラマが書いただけの場合もあるが。人が増えるとへぼいプログラマが入るのは例外というよりは必然だ）。
この場合、一度に考えられる問題の領域は大きい方が良い。
というか一定規模より上のコードを把握するのはある種の特殊能力を要求されるもので、
ベテランプログラマの中でも出来る人と出来ない人が現れるラインと思う。
ただ訓練すれば誰でもある程度は身につけられる能力とは思うが。

あと、問題の複雑さが本質的に結構な記述量を要求するような時も、
